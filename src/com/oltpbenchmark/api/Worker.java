...
import com.meetup.memcached.SockIOPool.SockIO;
...
public abstract class Worker<T extends BenchmarkModule> implements Runnable {
    ...
    public final T benchmarkModule;
    ...
    public final void run() {
        ...
        if (Config.ENABLE_LOGGING) {
            if (this instanceof SmallBankWorker) {
                SmallBankWorker w = (SmallBankWorker) this;
                try {
                    w.readLogAll.flush();
                    w.readLogAll.close();
                    w.updateLogAll.flush();
                    w.updateLogAll.close();
                } catch (Exception e) {
        ...
        if (completed.incrementAndGet() == getNumWorkers()) {
            try {
                if (Config.CAFE) {
                    if (this instanceof TPCCWorker) { 
                        ((TPCCWorker)this).cafe.clean();  

                        //                    TPCCCacheStore.avrStats();
                    } else if (this instanceof SmallBankWorker) {
                        ((SmallBankWorker)this).cafe.clean();
                    }
                }

                SockIOPool.getInstance(Config.CACHE_POOL_NAME).shutDown();
                
                com.meetup.memcached.Stats.aggregate();

                if (SmallBankConstants.STATS || TPCCConstants.STATS) {
                    JSONObject jobj = Stats.getAllStats();
