...
public class DBWorkload {
    ...
    public static void main(String[] args) throws Exception {
        ...
        options.addOption("ng", "ngcache", true, "Whether using ngcache for caching or not");
        options.addOption("po", "polygraph", true, "Generating logs for polygraph");
        options.addOption("st", "stats", true, "Enable stats or not.");
        
        options.addOption("mc", "caches", true, "The list of the cache servers");
        options.addOption("cm", "cachemode", true, "Either write-around, write-through or write-back");
        options.addOption("ba", "batch", true, "Batching changes");
        options.addOption("ar", "numarworkers", true, "Number of background worker threads");
        options.addOption("th", "threads", true, "Number of foreground threads.");
        options.addOption("sf", "scalefactor", true, "Number of warehouses");
        options.addOption("arsl", "arsleep", true, "Sleep time between applying session batch");
        options.addOption("minw", "minwhid", true, "Min warehouse id");
        options.addOption("maxw", "maxwhid", true, "Max warehouse id");
        options.addOption("dpw", "distperwhse", true, "Number of districts per warehouse");
        options.addOption("cpd", "custperdist", true, "Number of customers per district");
        options.addOption("war", "warmup", true, "Warmup up using write-through");
        options.addOption("rep", "replicas", true, "The number of replicas");
        options.addOption("ses", "storesess", true, "Storing the commited sessions to the database or not.");
        options.addOption("bp", "buffparallel", true, "Send buffered write in parallel?");
        options.addOption("mwu", "manualwarmup", true, "Doing the manual warmup or not.");
        options.addOption("pm", "persistmode", true, "The cache persistent mode.");
        ...
        if (argsLine.hasOption("ngcache")) {
            Config.CAFE = Boolean.parseBoolean(argsLine.getOptionValue("ngcache"));
        } else {
            Config.CAFE = false;
        }
        
        if (argsLine.hasOption("polygraph")) {
            Config.ENABLE_LOGGING = Boolean.parseBoolean(argsLine.getOptionValue("polygraph"));
        } else {
            Config.ENABLE_LOGGING = false;
        }
        
        if (argsLine.hasOption("arsleep")) {
            Config.AR_SLEEP = Integer.parseInt(argsLine.getOptionValue("arsleep"));
        } 
        
        if (argsLine.hasOption("cachemode")) {
            String mode = argsLine.getOptionValue("cachemode");
            switch (mode) {
            case "around":
                Config.CACHE_POLICY = CachePolicy.WRITE_AROUND;
                break;
            case "through":
                Config.CACHE_POLICY = CachePolicy.WRITE_THROUGH;
                break;
            case "mix":
                Config.CACHE_POLICY = CachePolicy.MIX;
                break;
            case "back":
            default:
        ...
        if (argsLine.hasOption("ar")) {
            Config.NUM_AR_WORKERS = Integer.parseInt(argsLine.getOptionValue("ar"));
        }
        
        if (argsLine.hasOption("ba")) {
            Config.BATCH = Integer.parseInt(argsLine.getOptionValue("ba"));
        }
        
        if (argsLine.hasOption("mc")) {
            String caches = argsLine.getOptionValue("mc");
            Config.cacheServers = caches.split(",");
        }       
        
        int threads = 0;
        if (argsLine.hasOption("threads")) {
            threads = Integer.parseInt(argsLine.getOptionValue("threads"));
        }
        
        int scaleFactor = 1;
        if (argsLine.hasOption("scalefactor")) {
            scaleFactor = Integer.parseInt(argsLine.getOptionValue("scalefactor"));
        }
        
        if (argsLine.hasOption("distperwhse")) {
            TPCCConfig.configDistPerWhse = Integer.parseInt(argsLine.getOptionValue("distperwhse"));
        }
        
        if (argsLine.hasOption("custperdist")) {
            TPCCConfig.configCustPerDist = Integer.parseInt(argsLine.getOptionValue("custperdist"));
        }        
        
        if (argsLine.hasOption("warmup")) {
            MonitorThread.warmupThreshold = Double.parseDouble(argsLine.getOptionValue("warmup"));
        }
        
        if (argsLine.hasOption("replicas")) {
            com.usc.dblab.cafe.Config.replicas = Integer.parseInt(argsLine.getOptionValue("replicas"));
        }
        
        if (argsLine.hasOption("storesess")) {
            com.usc.dblab.cafe.Config.storeCommitedSessions = Boolean.parseBoolean(argsLine.getOptionValue("storesess"));
        }
        
        if (argsLine.hasOption("buffparallel")) {
            com.usc.dblab.cafe.Config.sendParallel = Boolean.parseBoolean(argsLine.getOptionValue("buffparallel"));
        }
        
        if (argsLine.hasOption("manualwarmup")) {
            TPCCConfig.warmup = Boolean.parseBoolean(argsLine.getOptionValue("manualwarmup"));
        }
        
        if (argsLine.hasOption("persistmode")) {
            String persistMode = argsLine.getOptionValue("persistmode");
            switch (persistMode) {
            case "async_bdb":
                com.usc.dblab.cafe.Config.persistMode = com.usc.dblab.cafe.Config.BDB_ASYNC;
                break;
            case "sync_bdb":
                com.usc.dblab.cafe.Config.persistMode = com.usc.dblab.cafe.Config.BDB_SYNC;
                break;
            case "no_persist":
            default:
                com.usc.dblab.cafe.Config.persistMode = com.usc.dblab.cafe.Config.NO_PERSIST;
                break;
            }
        }
        
        System.out.println("Number of districts per warehouse: "+TPCCConfig.configDistPerWhse);
        System.out.println("Number of customers per district: "+TPCCConfig.configCustPerDist);
        System.out.println("Number of replicas R = "+com.usc.dblab.cafe.Config.replicas);
        System.out.println("Use CommitedSession table = "+com.usc.dblab.cafe.Config.storeCommitedSessions);
        System.out.println("Sending buff in parallel = "+com.usc.dblab.cafe.Config.sendParallel);
        System.out.println("Manual warmup = "+TPCCConfig.warmup);
        System.out.println("Persist mode = "+com.usc.dblab.cafe.Config.persistMode);
        
        // ========================= Write-back Configs =============== //
        System.out.println("################## Version 2");
        System.out.println("NgCache = "+Config.CAFE);
        if (Config.CAFE) {
            System.out.println("Cache Mode = "+Config.CACHE_POLICY);
            System.out.println("Cache servers = "+Arrays.toString(Config.cacheServers));
            System.out.println("Num ARs = "+Config.NUM_AR_WORKERS);
            System.out.println("Batch = "+Config.BATCH);
            System.out.println("AR sleep = "+Config.AR_SLEEP);
        }
       
        boolean mix = false;
        if (Config.CACHE_POLICY == CachePolicy.MIX) {
            // initially, write-through
            Config.CACHE_POLICY = CachePolicy.WRITE_THROUGH;
            mix = true;
        }
        ...
        // start a thread to monitor the cache hit rate
        String url = xmlConfig.getString("DBUrl");
        String user = xmlConfig.getString("username");
        String pass = xmlConfig.getString("password");
...
//        if (Config.CAFE && TPCCConfig.warmup) {
        ...
        if(Config.CAFE ) {
            ...
            tokens[1] = Arrays.toString(Config.cacheServers).replaceAll(" ", "")
...
//            	ReadOnly.main(tokens);
        ...
        if (mix)
            new MonitorThread(url, user, pass).start();
...
//            wrkld.setCache(xmlConfig.getString("cache", "localhost:11211"));   
//            wrkld.setCafe(xmlConfig.getString("cafe", "false"));
//            wrkld.setLogging(xmlConfig.getString("logging", "true"));
//            wrkld.setCachePolicy(xmlConfig.getString("cachepolicy", "WRITE_THROUGH"));
            wrkld.setScaleFactor(scaleFactor);
            
            if (argsLine.hasOption("minwhid")) {
                wrkld.setMinWarehouseId(Integer.parseInt(argsLine.getOptionValue("minwhid")));
            } else {
                wrkld.setMinWarehouseId(1);
            }
            
            if (argsLine.hasOption("maxwhid")) {
                wrkld.setMaxWarehouseId(Integer.parseInt(argsLine.getOptionValue("maxwhid")));
            } else {
                wrkld.setMaxWarehouseId((int)wrkld.getScaleFactor());
            }
